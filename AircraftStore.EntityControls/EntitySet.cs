...
namespace EFWinforms
{
        ...
        IQueryable
    {
        ...
        internal void CancelChanges()
        {
            ...
            {
                var ctx = ((IObjectContextAdapter) DataSource.DbContext).ObjectContext;
        ...
        private IBindingListView GetBindingList()
        {
            if (_list == null)
            {
                ...
                _list =
                    (IEntityBindingList)
                        Activator.CreateInstance(listType, DataSource, Query, Guid.NewGuid().ToString()); // this.Name);
        ...
        private ListDictionary BuildLookupDictionary(IEnumerable entities)
        {
            ...
            {
                var list = (from object item in entities select new KvPair(item, item.ToString())).ToList();
            ...
            if (atts.Length > 0)
            {
                ...
                if (dpa != null)
                {
                    ...
                    {
                        var list =
                            (from object item in entities select new KvPair(item, (string) pi.GetValue(item, null)))
                                .ToList();
                        return BuildLookupDictionary(list);
                    }
                }
            ...
            return (from pi in ElementType.GetProperties()
                where pi.PropertyType == typeof (string)
                where
                    pi.Name.IndexOf("Name", StringComparison.OrdinalIgnoreCase) > -1 ||
                    pi.Name.IndexOf("Description", StringComparison.OrdinalIgnoreCase) > -1
                select (from object item in entities select new KvPair(item, (string) pi.GetValue(item, null))).ToList()
                into list
                select BuildLookupDictionary(list)).FirstOrDefault();

            // no dice
        }

        private ListDictionary BuildLookupDictionary(List<KvPair> list)
        {
        ...
        private class KvPair : IComparable
        {
            ...
            {
                return string.Compare(Value, ((KvPair) obj).Value, StringComparison.OrdinalIgnoreCase);
            }
        }
